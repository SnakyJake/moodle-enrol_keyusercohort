 ...
 */
class enrol_keyusercohort_handler {
    /**
            ...
            if ($instance->customint2) {
                if (!groups_is_member($instance->customint2, $event->relateduserid)) {
                    if ($group = $DB->get_record('groups', array('id'=>$instance->customint2, 'courseid'=>$instance->courseid))) {
                        groups_add_member($group->id, $event->relateduserid, 'enrol_keyusercohort', $instance->id);
                    }
                }
            }
        }
        ...
        foreach ($instances as $instance) {
            ...
            if ($unenrolaction == ENROL_EXT_REMOVED_UNENROL) {
                ...
                if ($ue->status != ENROL_USER_SUSPENDED) {
                    ...
                    role_unassign_all(array('userid'=>$ue->userid, 'contextid'=>$context->id, 'component'=>'enrol_keyusercohort', 'itemid'=>$instance->id));
                }
            }
        }
    ...
    public static function deleted(\core\event\cohort_deleted $event) {
        ...
        foreach ($instances as $instance) {
            if ($unenrolaction == ENROL_EXT_REMOVED_SUSPENDNOROLES) {
                $context = context_course::instance($instance->courseid);
                role_unassign_all(array('contextid'=>$context->id, 'component'=>'enrol_keyusercohort', 'itemid'=>$instance->id));
 ...
 */
function enrol_keyusercohort_sync(progress_trace $trace, $courseid = NULL) {
    global $CFG, $DB;
    require_once("$CFG->dirroot/group/lib.php");
    ...
    $sql = "SELECT e.roleid, ue.userid, c.id AS contextid, e.id AS itemid, e.courseid
         ...
         LEFT JOIN {role_assignments} ra ON (ra.contextid = c.id AND ra.userid = ue.userid AND ra.itemid = e.id AND ra.component = 'enrol_keyusercohort' AND e.roleid = ra.roleid)
    ...
    $rs = $DB->get_recordset_sql($sql, $params);
    foreach($rs as $ra) {
        role_assign($ra->roleid, $ra->userid, $ra->contextid, 'enrol_keyusercohort', $ra->itemid);
    ...
    $rs = $DB->get_recordset_sql($sql, $params);
    foreach($rs as $ra) {
        role_unassign($ra->roleid, $ra->userid, $ra->contextid, 'enrol_keyusercohort', $ra->itemid);
        $trace->output("unassigning role: $ra->userid ==> $ra->courseid as ".$allroles[$ra->roleid]->shortname, 1);
    }
    $rs->close();

    // Finally sync groups.
    $affectedusers = groups_sync_with_enrolment('keyusercohort', $courseid);
