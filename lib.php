 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_keyusercohort_plugin extends enrol_plugin {
    ...
    public function can_delete_instance($instance) {
        ...
        return has_capability('enrol/keyusercohort:config', $context);
    }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $coursecontext) or !has_capability('enrol/keyusercohort:config', $coursecontext)) {
            return false;
        }
        return keyuser_cohort_get_available_cohorts($coursecontext, 0, 0, 1) ? true : false;
    }
    ...
    public function add_instance($course, array $fields = null) {
        ...
        enrol_keyusercohort_sync($trace, $course->id);
        $trace->finished();
    ...
    public function update_instance($instance, $data) {
        ...
        require_once("$CFG->dirroot/enrol/keyusercohort/locallib.php");
        $trace = new null_progress_trace();
        enrol_keyusercohort_sync($trace, $instance->courseid);
        $trace->finished();
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $coursecontext) {
        ...
        $mform->addElement('select', 'roleid', get_string('assignrole', 'enrol_keyusercohort'), $roles);
        $mform->setDefault('roleid', $this->get_config('roleid'));
        $groups = $this->get_group_options($coursecontext);
        $mform->addElement('select', 'customint2', get_string('addgroup', 'enrol_keyusercohort'), $groups);
    }
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        if ($DB->record_exists_select('enrol', $sql, $params)) {
            $errors['roleid'] = get_string('instanceexists', 'enrol_keyusercohort');
        }
 ...
 * @return bool
 */
function enrol_keyusercohort_allow_group_member_remove($itemid, $groupid, $userid) {
    return false;
}
 ...
 */
function enrol_keyusercohort_create_new_group($courseid, $cohortid) {
    global $DB, $CFG;
    ...
    $a->name = $groupname;
    $a->increment = '';
    $groupname = trim(get_string('defaultgroupnametext', 'enrol_keyusercohort', $a));
    $inc = 1;
    ...
    while ($DB->record_exists('groups', array('name' => $groupname, 'courseid' => $courseid))) {
        $a->increment = '(' . (++$inc) . ')';
        $newshortname = trim(get_string('defaultgroupnametext', 'enrol_keyusercohort', $a));
