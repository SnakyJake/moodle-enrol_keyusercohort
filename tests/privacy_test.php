...
/**
 * Base class for unit tests for enrol_keyusercohort.
 *
...
use enrol_keyusercohort\privacy\provider;

/**
 * Unit tests for the enrol_keyusercohort implementation of the privacy API.
 *
 * @copyright  2018 Carlos Escobedo <carlos@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_keyusercohort_privacy_testcase extends \core_privacy\tests\provider_testcase {
    ...
    public function test_export_user_data() {
        ...
        $this->setUser($user1);
        $contextlist = provider::get_contexts_for_userid($user1->id);
        $approvedcontextlist = new approved_contextlist($user1, 'enrol_keyusercohort', $contextlist->get_contextids());
        provider::export_user_data($approvedcontextlist);
        foreach ($contextlist as $context) {
            $writer = writer::with_context($context);
            $data = $writer->get_data([
                get_string('pluginname', 'enrol_keyusercohort'),
                get_string('groups', 'core_group')
            ]);
            $this->assertTrue($writer->has_any_data());
            if ($context->contextlevel == CONTEXT_COURSE) {
                $exportedgroups = $data->groups;
                // User1 only belongs to group1 via enrol_keyusercohort.
    ...
    public function test_delete_data_for_user() {
        ...
        $approvedcontextlist = new \core_privacy\tests\request\approved_contextlist($user1, 'enrol_keyusercohort',
    ...
    public function test_delete_data_for_users() {
        ...
        $approveduserlist = new \core_privacy\local\request\approved_userlist($coursecontext1, 'enrol_keyusercohort',
                [$user1->id, $user2->id]);
    ...
    public function test_get_users_in_context() {
        ...
        $userlist = new \core_privacy\local\request\userlist($context, 'enrol_keyusercohort');
        \enrol_keyusercohort\privacy\provider::get_users_in_context($userlist);
